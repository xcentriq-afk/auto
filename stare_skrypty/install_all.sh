...
do_snapped() {
  ...
  info "Kontrole końcowe:"
  info "- snapper root list:"
  snapper -c root list || true

  if mountpoint -q /home; then
    info "- snapper home list (jeśli skonfigurowane):"
    snapper -c home list || true
  fi

  info "- przeszukuję grub.cfg w poszukiwaniu wpisów snapshot (jeśli grub-btrfs dodał):"
  if command -v grub-mkconfig >/dev/null 2>&1; then
